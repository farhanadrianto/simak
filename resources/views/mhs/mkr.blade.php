...
<div class="section-title">Mata Kuliah Reguler (MKR)</div>
...
<div class="alert-custom">
    📌 Silakan tentukan pilihan mata kuliah reguler program studi Anda, kemudian tekan tombol Simpan di bawah tabel.
</div>
            ...
            <tbody>
                @foreach($matkul as $i => $m)
                    ...
                    <tr @if($sudahAmbil) style="opacity: 0.5; background: #f8fafc;" @endif>
                        ...
                        <td style="font-weight: 600; color: #475569;">{{ $m->kode_matkul }}</td>
                        <td style="font-weight: 600; color: #0f172a;">{{ $m->nama_matkul }}</td>
                        <td><b class="sks-val" style="color: #0f172a;">{{ $m->sks }}</b></td>
                        <td style="font-weight: 500;">{{ $m->kelas }}</td>
                        <td>
                            <span class="badge {{ $penuh ? 'penuh' : '' }}">
                                {{ $m->jumlah_terisi }}/{{ $m->kapasitas }}
                            </span>
                        </td>
                        <td style="font-weight: 500;">{{ $m->hari }}</td>
                        <td style="color: #475569; font-size: 13px;">{{ $m->jam_mulai }} - {{ $m->jam_selesai }}</td>
